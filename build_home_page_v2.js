...
export default function StudentDashboard() {
  ...
  // New Header for Home Page (White Background, Slimmer)
  const HomeHeader = () => (
    <div className="bg-white px-5 pt-3 pb-3 flex justify-between items-center z-50 relative rounded-b-[24px] shadow-[0_4px_15px_rgba(0,0,0,0.04)]">
        <div className="flex items-center gap-2.5">
            <div className="w-9 h-9 bg-slate-50 rounded-full flex items-center justify-center p-1 border border-slate-100">
            ...
            <div className="flex flex-col">
                ...
                <span className="text-[10px] font-bold text-slate-500">Class 10</span>
            </div>
        </div>
        
        <div className="flex items-center gap-2.5">
            <div className="flex items-center gap-1 bg-orange-50 px-2 py-0.5 rounded-full border border-orange-100 shadow-sm cursor-pointer hover:bg-orange-100 transition-colors">
                <Flame size={12} className="text-orange-500 fill-orange-500" />
                <span className="text-[11px] font-black text-orange-600">12</span>
            </div>
            <div className="w-8 h-8 rounded-full border-2 border-amber-400 overflow-hidden shadow-sm cursor-pointer transform hover:scale-105 transition-transform">
  ...
  const LeftScreen = () => {
      ...
      return (
         ...
         >
            ...
            <div className="absolute top-12 left-0 w-full h-[60vh] z-0 overflow-hidden">
            ...
            {/* Welcome & Mascot */}
            <div className="relative z-20 flex flex-col items-center mt-4 flex-1">
                ...
                <motion.div 
                    ...
                    className="mt-6 w-36 h-36 transform hover:scale-110 transition-transform cursor-pointer relative z-30 drop-shadow-[0_10px_20px_rgba(0,0,0,0.15)]"
                >
                    <img src="/owl1.png" alt="Owl Mascot" className="w-full h-full object-contain" />
                </motion.div>
            </div>

            {/* Bottom White overlapping card (pb-2 instead of pb-10 to remove bottom gap) */}
            <div className="relative z-20 bg-white rounded-t-[40px] px-5 pt-8 pb-3 shadow-[0_-15px_40px_rgba(0,0,0,0.08)] w-full mt-auto">
                ...
                {/* School Category Grid (5 items per row) */}
                <div className="flex justify-between items-center mb-5 mt-2">
                    <h3 className="text-base font-black text-slate-800">School Category</h3>
                </div>
                <div className="grid grid-cols-5 gap-y-5 gap-x-1 mb-8">
                    {categories.map((cat, i) => (
                        <div key={i} className="flex flex-col items-center gap-1.5 cursor-pointer group" onClick={() => setActiveTab(cat.id)}>
                            <div className={\`w-11 h-11 rounded-[16px] flex items-center justify-center border shadow-[0_4px_10px_rgba(0,0,0,0.03)] group-hover:scale-110 transition-all duration-300 \${cat.color}\`}>
                                <cat.icon size={18} strokeWidth={2.5} />
                            </div>
                            <span className="text-[8px] font-black text-slate-600 text-center leading-tight whitespace-nowrap tracking-tight">{cat.name}</span>
                        </div>
                    ))}
                </div>
                ...
                <div className="mt-8 border-t border-slate-100 pt-8">
                    ...
                    {/* Podium Top 3 (Vibrant Colors) */}
                    <div className="flex justify-center items-end gap-3 mb-8 pt-4">
                        {/* 2nd Place */}
                        <div className="flex flex-col items-center flex-1">
                            <div className="relative">
                                <div className="w-14 h-14 rounded-full border-[4px] border-[#A2D2FF] overflow-hidden bg-slate-50 shadow-[0_4px_15px_rgba(162,210,255,0.4)]"><img src="https://api.dicebear.com/7.x/notionists/svg?seed=Rahul&backgroundColor=transparent" alt="2nd" className="w-full h-full object-cover" /></div>
                                <div className="absolute -bottom-2 -right-1 bg-[#A2D2FF] text-blue-900 w-6 h-6 rounded-full flex items-center justify-center text-[11px] font-black border-2 border-white shadow-sm">2</div>
                            </div>
                            <span className="text-[11px] font-black mt-3 text-slate-700 truncate w-full text-center">Rahul</span>
                            <span className="text-[9px] text-[#A2D2FF] font-black">450 pts</span>
                            <div className="w-full h-16 bg-gradient-to-t from-[#EAF4FF] to-white rounded-t-2xl mt-2 border-t-[3px] border-[#A2D2FF] flex justify-center pt-2 shadow-inner">
                                <div className="w-6 h-1 rounded-full bg-[#A2D2FF]/50" />
                            </div>
                        </div>
                        {/* 1st Place */}
                        <div className="flex flex-col items-center flex-1 relative -top-6">
                            <motion.div animate={{ y: [0, -5, 0] }} transition={{ duration: 2, repeat: Infinity }} className="absolute -top-7 text-4xl drop-shadow-md z-10">👑</motion.div>
                            <div className="relative">
                                <div className="w-20 h-20 rounded-full border-[4px] border-[#FFD166] overflow-hidden bg-amber-50 shadow-[0_4px_20px_rgba(255,209,102,0.5)]"><img src="https://api.dicebear.com/7.x/notionists/svg?seed=Aryan&backgroundColor=transparent" alt="1st" className="w-full h-full object-cover" /></div>
                                <div className="absolute -bottom-2 right-0 left-0 mx-auto bg-[#FFD166] text-amber-900 w-7 h-7 rounded-full flex items-center justify-center text-[12px] font-black border-2 border-white shadow-sm">1</div>
                            </div>
                            <span className="text-sm font-black mt-3 text-amber-500 truncate w-full text-center">Aryan</span>
                            <span className="text-[10px] text-amber-400 font-black">520 pts</span>
                            <div className="w-full h-24 bg-gradient-to-t from-[#FFF9E6] to-white rounded-t-2xl mt-2 border-t-[3px] border-[#FFD166] flex justify-center pt-2 shadow-[inset_0_4px_10px_rgba(255,209,102,0.1)]">
                                <Star size={18} className="text-[#FFD166] fill-[#FFD166]" />
                            </div>
                        </div>
                        {/* 3rd Place */}
                        <div className="flex flex-col items-center flex-1">
                            <div className="relative">
                                <div className="w-14 h-14 rounded-full border-[4px] border-[#FFA69E] overflow-hidden bg-red-50 shadow-[0_4px_15px_rgba(255,166,158,0.4)]"><img src="https://api.dicebear.com/7.x/notionists/svg?seed=Neha&backgroundColor=transparent" alt="3rd" className="w-full h-full object-cover" /></div>
                                <div className="absolute -bottom-2 -left-1 bg-[#FFA69E] text-red-900 w-6 h-6 rounded-full flex items-center justify-center text-[11px] font-black border-2 border-white shadow-sm">3</div>
                            </div>
                            <span className="text-[11px] font-black mt-3 text-slate-700 truncate w-full text-center">Neha</span>
                            <span className="text-[9px] text-[#FFA69E] font-black">390 pts</span>
                            <div className="w-full h-12 bg-gradient-to-t from-[#FFF0EF] to-white rounded-t-2xl mt-2 border-t-[3px] border-[#FFA69E] flex justify-center pt-2 shadow-inner">
                                <div className="w-6 h-1 rounded-full bg-[#FFA69E]/50" />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-2">
                        ...
                        <button className="w-full py-4 mt-3 bg-slate-50 text-xs font-black text-slate-500 hover:bg-[#FFF9E6] hover:text-amber-600 hover:border-[#FFD166]/50 rounded-[20px] transition-all border border-slate-100 shadow-sm">
                            View All Rankings
                        </button>
                    </div>
                </div>

                {/* Footer Branding (mb-0 and pb-2 so no extra scroll space) */}
                <div className="mt-10 mb-0 flex flex-col items-center justify-center text-center space-y-1">
                    <p className="text-[10px] font-bold text-slate-400">Student Portal v2.0</p>
  ...
  const RightScreen = () => {
    // keeping original RightScreen content with minor fixes
    const renderContent = () => {
        if (activeTab === "diary") {
            return (
